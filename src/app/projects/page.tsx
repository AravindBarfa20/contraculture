...
const statusConfig: Record<string, { color: string; bg: string; label: string }> = {
  draft: { color: "text-zinc-400", bg: "bg-zinc-900 border-zinc-800", label: "DRAFT" },
  analyzed: { color: "text-zinc-300", bg: "bg-zinc-800 border-zinc-700", label: "ANALYZED" },
  adapted: { color: "text-zinc-200", bg: "bg-zinc-800 border-zinc-700 font-bold", label: "ADAPTED" },
  simulated: { color: "text-white", bg: "bg-zinc-800/90 border-zinc-600 font-extrabold shadow-sm", label: "SIMULATED" },
};
    ...
    <div className="relative min-h-screen bg-zinc-950 overflow-hidden font-sans text-zinc-100">
      ...
      <main className="relative container mx-auto px-4 py-8 max-w-6xl">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="flex items-center justify-between mb-8 flex-wrap gap-4">
          <div>
            <h2 className="text-2xl font-extrabold tracking-tight text-white">Project Workspace</h2>
            <p className="text-xs font-mono text-zinc-400 mt-1">
              Manage your landing page copy and cultural adaptation pipelines
            </p>
          </div>
          <Button onClick={() => router.push("/projects/new")} className="bg-gradient-to-b from-zinc-100 to-zinc-300 hover:from-white hover:to-zinc-200 text-zinc-950 font-mono text-xs font-bold rounded-xl shadow-md border border-white/40">
            <Plus className="w-3.5 h-3.5 mr-1.5 text-zinc-950" />+ new_project
          </Button>
        </motion.div>
        ...
        {loading ? (
          ...
          <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }}>
            <Card className="text-center py-16 border border-dashed border-zinc-800 bg-zinc-950/80 backdrop-blur-sm rounded-2xl">
              <CardContent>
                <div className="w-14 h-14 rounded-xl bg-zinc-900 border border-zinc-700 flex items-center justify-center mx-auto mb-4 font-mono text-xl text-zinc-300">
                  🌍
                </div>
                <h3 className="text-lg font-bold text-white mb-1">No Projects Found</h3>
                <p className="text-xs text-zinc-400 mb-6 max-w-sm mx-auto font-mono">Create your first project to start adapting copy across global markets</p>
                <Button onClick={() => router.push("/projects/new")} className="bg-gradient-to-b from-zinc-100 to-zinc-300 hover:from-white text-zinc-950 font-mono text-xs font-bold rounded-xl shadow-md">
            ...
            {projects.map((project, index) => {
              ...
              return (
                <motion.div key={project.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25, delay: index * 0.05 }}>
                  <Card className="cursor-pointer group transition-all duration-200 border border-zinc-800 hover:border-zinc-500 hover:shadow-xl bg-zinc-950 rounded-xl overflow-hidden" onClick={() => router.push(`/projects/${project.id}`)}>
                    <CardHeader className="pb-3 pt-5 px-5">
                      <div className="flex items-center justify-between mb-1">
                        <CardTitle className="text-base font-bold text-white group-hover:text-zinc-200 transition-colors">{project.name}</CardTitle>
                      ...
                      <CardDescription className="line-clamp-2 text-xs text-zinc-400 leading-relaxed">{project.description}</CardDescription>
                    </CardHeader>
                    <CardContent className="pb-5 px-5">
                      <div className="flex items-center justify-between pt-3 border-t border-zinc-800/80">
                        ...
                        <div className="flex items-center text-xs font-mono text-zinc-400 group-hover:text-white transition-colors font-medium">
